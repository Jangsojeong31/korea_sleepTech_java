...
/*
	...
	2. 응집도 (높은 응집도)
		: 하나의 클래스나 모듈 안에서 관련된 작업들끼리 모여 있는가
			한 모듈 내의 구성요소들 간의 연관정도
...
>> 클래스 간의 의존성을 줄이고, 변경에 강한 구조 (변경을 하더라도 서로 영향을 미치지 않도록)
...
class Classroom {
	...
	// : 클래스가 직접 객체를 생성하지 x,  ex) teacher = new KoreanTeacher();
	// 외부에서 객체를 주입받도록 설계
...
class Attendance { // 출석부는 출석체크 기능만 담당
