...
/*
  ...
  cf) 기본 데이터 타입
  		...
  		2) 언박싱
  			: Wrapper >> 기본 타입으로 변하는 과정
  			
  		// 장단점
  		장점 : 기본 타입과 객체 타입 간의 호환이 용이
  		단점 : 객체 과정에서 메모리 사용량 증가(박싱, 언박싱 과정에서 성능 저하)
...
public class A_Wrapper {
	public static void main(String[] args) {
		...
		System.out.println(iObject.toString()); // 5
...
//		cf) Wrapper 클래스 내부의 toString() 메서드는 객체가 보유하고 있는 실제 값이 출력
		...
		// 2) 언박싱
		Integer integerVaue = new Integer(15);
		int iValue = integerVaue;
...
//		new Integer("hello"); 
		...
		// 2) intValue() : Integer 객체를 기본 int 타입으로 변환
		int myInt = myInteger.intValue();
		System.out.println(myInt);
		
		// 3) parseInt(String s) : 문자열을 기본 int 타입으로 파싱
		// 변환 과정 : String >> Integer >> int
		int parseInt = Integer.parseInt("123");
		System.out.println(parseInt); // 123
		
		// 4) compareTo(Integer anotherInt) : 두 Integer 객체를 비교 => 같으면 0, 호출 객체(a)가 더 크면 1, 더 작으면 -1
		Integer anotherInt = new Integer(49);
