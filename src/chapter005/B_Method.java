...
/*
 	...
 	+) 구현부
 	: 반환타입이 void가 아닐 경우 반드시! 'return 반환타입데이터;' 포함해야 함!
 	>> 반환값은 명시된 반환타입과 일치하거나 '자동 형변환이 가능한 데이터'
...
class Example {
	...
	int add(int x, int y) { // 파라미터(매개 변수) - 지역 변수
		int result = x + y; // 지역 변수(result : 해당 메서드 내에서만 유효)
		return result;
	
	}
	
	double multipy() {
		int result = x * y; // 인스턴스 변수를 받아 사용
		return result;
	}
...
public class B_Method {
	public static void main(String[] args) {
		...
		// 함수의 파라미터에 데이터 전달
		// : 함수 호출 시 () 영역 내에서 parameter(매개 변수)에 전달되는 값을 argument라고 부름
...
//		example.add(1); -> 인자값과 매개변수의 수는 같아야 한다.
//		example.add(1, 2, 3);
		
//		example.add("1", "2"); -> 인자값과 매개변수의 타입은 같거나 자동 형변환이 가능해야 한다.
		
		example.add('1','2');  // -> 자동 형변환 가능(char -> int)
