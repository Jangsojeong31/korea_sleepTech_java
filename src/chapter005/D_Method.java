...
class Operator {
	...
	static int subtract(int a, int b) {
		...
		// cf) 필드값과 지역변수의 명이 동일할 때;
		// 인스턴스 메서드에서 필드값을 지정하고자 하는 경우 this 키워드 사용
//		int result = this.a - this.b;
	...
	int multiply(int a, int b) {
		int result = this.a - this.b; // 필드값에 접근(this가 없으면 지역변수로 접근함)
		return result;
	}
}
...
public class D_Method {
	public static void main(String[] args) {
		...
		// 인스턴스화 없이 메서드 호출 가능
		int result = Operator.subtract(10, 20); //클래스명.메서드명(); (객체명.메서드명()도 가능하지만, 클래스명.메서드명() 권장)
