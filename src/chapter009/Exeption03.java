...
/*
  	...
  	1. throw
  		...
  		- 사용법 : throw new ExceptionType("예외 메시지");
  		>> 인자로 전달되는 문자열 "예외 메시지"는 getMessage() 메서드를 통해 catch 블록에서 출력 가능
  		
	2. throws
		: 메서드 '선언부'에 위치, 호출자에게 예외 처리를 위임
		- 체크(컴파일) 예외 : 필수 / 언체크(런타임) 예외 : 선택
		- 사용법 : [접근제어자] 반환타입 메서드명() throws ExceptionType {}
		
		cf) throw : 메서드 '내부'에 위치, 예외를 발생시킴
		
		
	사용자 정의 예외
	: 자바의 Exception 클래스를 '상속'받아 자식 클래스를 직접 정의 
 */
...
public class Exeption03 {
	...
	static void login(String username, String password) throws InvalidUserException {
		// 문자열.equals(문자열);
		// : 동일한 문자열일 경우 true, 그렇지 않으면 false 값 반환
		if (!"admin".equals(username)) { // username이 admin이 아닐 경우(false) -> true
			// 잘못된 사용자 인 경우
...
//		login("admin", "password"); => 오류 발생 : try-catch 블록의 예외 처리가 필수
		try {
