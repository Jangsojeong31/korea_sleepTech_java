// == 패키지 == //
// : 폴더 (파일 분리 저장소)
package chapter003; // chapter03 폴더 내의 파일임을 명시
...
public class A_Array { // 클래스명 == 파일명 (대문자로 시작) 
	...
	public static void main(String[] args) {
		...
		// 데이터를 연속적으로 저장
		// 순서가 있음 (index 인덱스 번호, 0부터 1씩 증가되는 값)
		...
		int[] scores01;  // 정수형 데이터를 담을 수 있는 배열
		int scores02[];
		...
		scores01 = new int[3]; //요소의 개수가 3개인 배열 생성
		
		// cf) 참조 자료형 - 주소값 저장
		System.out.println(scores01);  // [I@372f7a8d
		
		// == 배열 요소에 접근 == //
		// : 배열명[인덱스 번호]
		// +) 데이터 변경(삽입) : 배열명[인덱스 번호] = 데이터값;
		System.out.println(scores01[0]);  
		System.out.println(scores01[1]);
		System.out.println(scores01[2]);
		...
		System.out.println(scores01[1]);
		System.out.println(scores01[2]);
//	    System.out.println(scores01[3]); -> 오류 발생 : ArrayIndexOutOfBoundsException(배열의 범위를 벗어남)
		
		// == 배열의 선언과 동시에 초기화 == //
		// : new 연산자 사용 + 직접적인 요소 대입의 배열 생성 시 길이 지정을 하지 않아도 됨
		...
		// 2) 데이터타입[] 배열명 = {요소1, 요소2, ...};
		char[] greet = {'h', 'e', 'l', 'l', 'o'}; //데이터 중복 가능
...
//		System.out.println(greet[5]); -> 오류 발생 : ArrayIndexOutOfBoundsException(배열의 범위를 벗어남)
		
		// == 배열의 길이(크기) == //
		// 배열명.length 속성으로 사용
		System.out.println(greet.length); // 출력 : 5
		...
		for (int index = 0; index < originalLength; index++) {
			if (index == removeIndex) {
				...
				// : index 번호 2번의 요소가 새로운 배열에서는 삭제됨
