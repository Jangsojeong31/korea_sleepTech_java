...
// >> 자식 클래스에서 생성자 호출 시 '항상' 부모 클래스의 생성자가 같이 호출!
// >> 반드시 사용자 정의 부모 생성자 호출일 필요 x (기본적으로 super();가 호출됨)
...
// cf) 접근제어자가 생략되어 있는 default 값의 경우
// 		: 같은 패키지 내에서 접근 가능
// 		>> 같은 패키지 내에서 같은 클래스명 사용 불가
//class Animal { -> 오류 발생 : chapter006 패키지 내에 같은 클래스명이 B_Inheritance에서 이미 사용 중
//	
//}

/*
  Cat 클래스(자식) - Mammal 클래스 상속받음
  1) 필드 : name
  2) 생성자 : 매개변수 o / x 생성자 - 오버로딩
...
//		System.out.println(this.name); 와 동일
...
//		// 빈 자식 생성자 - 인스턴스 생성
//		
		System.out.println(cat1.name); // Child
		
		
		
		Cat cat2 = new Cat("나비"); 
		System.out.println(cat2.name);  // 나비
		System.out.println(mammal1.name);  // Parent
		cat2.displayCat(); // 자식 이름 : 나비
						   // 부모 이름 : 나비
