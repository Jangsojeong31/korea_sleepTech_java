...
/*
 	...
 	1. 특징
 		1) 스트림은 데이터를 소스로부터 연속적으로 처리하는 '파이프 라인' 구축
...
public class G_Stream {
	public static void main(String[] args) {
		...
		System.out.println(names); // [이승아, 조승범, 진상영, 이은우]
		...
		// 3. 최종 연산
		// - forEach : 각 요소를 처리, 반환 없음
		// - collect : 요소를 변환하여 컬렉션으로 반환
		// - reduce : 모든 요소를 하나의 값으로 줄임
		// - count : 요소의 개수를 반환
		
		names.stream()  //1. 데이터 소스(Stream 생성)
			.map(name -> name + "님")  //2. 중간 연산
			.filter(name -> name.startsWith("이")) 
			// 문자열.startsWith("") : 해당 키워드로 시작하는 문자일 경우 true 를 반환
			.forEach(System.out::println); // 3. 최종 연산
//			.forEach(name -> System.out.println(name)); 를 위처럼 축약(메서드 참조)
		...
		// cf) .stream으로 생성된 스트림은 일회성
		// 		: 메서드 체이닝으로 여러 기능 적용
		//		: 스트림으로 생성된 데이터를 다시 스트림으로 변경 x
		
		Stream<Integer> stream = newNumbers.stream(); // 스트림으로 변경 후 저장
		stream.map(n -> n * n)
			.collect(Collectors.toList());
		
//		stream.filter(n -> n % 2 == 0)
//			.collect(Collectors.toList());
			// 스트림 재사용 시 예외 -> 스트림으로 생성된 데이터를 다시 스트림으로 변경 x 
		
		System.out.println(numbers); // [1, 2, 3, 4, 5] - 불변성!
