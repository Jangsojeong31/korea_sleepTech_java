...
/*
 	 ...
 	 >> 오름차순 정렬
 	 cf) 내림차순 정렬 - Comparator 클래스 사용 .reverseOrder() 정적 메서드 사용
...
//		names.sort(Comparator.comparingInt(String::length);
		...
		System.out.println(names);  // [Tom, Jerry, Loopy, Pororo] : 기존의 names 리스트가 바뀜

		
		// cf) sort() 메서드
		// : List 인터페이스의 메서드
		// - 리스트를 특정 기준에 따라 정렬
		// >> 졍렬의 기준을 결정하는 Comparator를 인수로 받음
		...
		// 3) 스트림 API 사용 정렬
		// : 정렬된 결과를 새로운 리스트로 반환
		List<Integer> listNumbers = new ArrayList<>(List.of(7,5,3, 4, 1));
		...
		System.out.println(characterNames);  // [Tom, Jerry, Pororo, Loopy] : 기존의 리스트는 바뀌지 않음
