...
/*
 	...
 	1. Optional 클래스
 		: null 아닌 값을 포함하거나, 값이 없는 상태를 나타낼 수 있음
 	...
 	3. Optional 의 주요 메서드
 		1) 값이 있는지 확인
 			- isPresent : 존재할 경우(true), 존재하지 않을 경우(false)
 			- isEmpty : 존재하지 않을 경우(true), 존재할 경우(false) 
		2) 값을 가져오기
	...
	cf)
	null 값일 때 -> NullPointerException
	빈 객체일 때 -> NoSuchElementException
...
//		System.out.println(optional3.get()); // NoSuchElementException
		...
		try {
			...
			System.out.println(e);  // java.lang.Error: 비워질 경우 사용자 정의 예외 발생
		}
		...
		// - 값이 반드시 존재해야 하는 경우는 Optional 사용 x
